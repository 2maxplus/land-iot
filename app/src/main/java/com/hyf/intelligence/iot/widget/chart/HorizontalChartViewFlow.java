package com.hyf.intelligence.iot.widget.chart;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for(int i = 6 ; i >= 0 ; i --){
            ...
            canvas.drawText(sdf.format(getBeforeOrAfterDate(new Date(), -(i+1))), lift_width - 10 ,
                    ((line_length / 7)) * (i + 1)  + (fm1.bottom - fm1.top) / 2 - 30, paint_font2);
            //比例图
            canvas.drawRect(line_width * (list.get(i).getStart()) + lift_width,
                    line_length / list.size() * (list.size() - i) - (line_length / 24 + plan_height / 2) + fm1.bottom,
                    line_width * (list.get(i).getEnd()) + lift_width,
                    line_length / list.size() * (list.size() - i) - (line_length / 24 + plan_height / 2) + plan_height + fm1.bottom, paint_plan);


            canvas.drawText(list.get(i).getTitls(),(line_width * (list.get(i).getEnd() -  list.get(i).getStart())) / 2 + line_width * (list.get(i).getStart()) + lift_width,line_length / list.size() * (list.size() - i) - line_length / 24  + (fm1.bottom-fm1.top)/2 ,paint_content);
            canvas.restore();
        }
